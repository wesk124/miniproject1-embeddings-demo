...
def load_sentence_transformer_model(model_name):
    sentenceTransformer = SentenceTransformer(model_name, device='cpu')
...
def get_sentence_transformer_embeddings(sentence, model_name="all-MiniLM-L6-v2"):
    ...
    try:
        emb = sentenceTransformer.encode(sentence)
        emb = np.array(emb, dtype=np.float64)

        # Helpful debug if something returns a zero vector unexpectedly
        if la.norm(emb) == 0.0:
            st.error(f"SentenceTransformer returned ZERO embedding for: {repr(sentence)}")

        return emb

    except Exception as e:
        st.error(f"SentenceTransformer encode FAILED for {repr(sentence)}: {e}")
        return np.zeros(384, dtype=np.float64) if model_name == "all-MiniLM-L6-v2" else np.zeros(512, dtype=np.float64)
...
def plot_piechart_helper(sorted_cosine_scores_items):
    ...
    # Always create fig/ax first so fig always exists
    fig, ax = plt.subplots(figsize=(3, 3))

    # Added this to prevent  NaN/Inf for all zeros
    if (not np.isfinite(sorted_cosine_scores).all()) or np.all(sorted_cosine_scores == 0):
        fig, ax = plt.subplots(figsize=(3, 3))
...
def cosine_similarity(x, y):
    ...
    # Numerical safety (optional but good)
    cos = max(-1.0, min(1.0, cos))

    # Exponentiate so the score is positive
    return float(np.exp(cos))
    #pass
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    ##################################
    # Basic tokenization: split + strip common punctuation
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        # 1) Input sentence embedding (GloVe avg)
        sent_embed = averaged_glove_embeddings_gdrive(
            st.session_state.text_search, word_index_dict, embeddings, model_type=model_type
        )

        # 2) Category embeddings cache (optional but helpful)
        cache_key = "cat_embed_glove_" + str(model_type)
        ...
        for i, cat in enumerate(categories):
            if cat not in st.session_state[cache_key]:
                st.session_state[cache_key][cat] = averaged_glove_embeddings_gdrive(
                    cat, word_index_dict, embeddings, model_type=model_type
                )
            cat_embed = st.session_state[cache_key][cat]
            cosine_sim[i] = cosine_similarity(sent_embed, cat_embed)
        ...
        # 1) Input sentence embedding (OpenAI)
        sent_embed = get_openai_embeddings(st.session_state.text_search, model_name=model_name)

        # 2) Category embeddings cache
        if cache_key not in st.session_state:
            st.session_state[cache_key] = {}
        ...
        for i, cat in enumerate(categories):
            if cat not in st.session_state[cache_key]:
                st.session_state[cache_key][cat] = get_openai_embeddings(cat, model_name=model_name)
            cat_embed = st.session_state[cache_key][cat]
            cosine_sim[i] = cosine_similarity(sent_embed, cat_embed)
        ...
        # 1) Input sentence embedding (Transformer)
        sent_embed = get_sentence_transformer_embeddings(st.session_state.text_search, model_name=model_name)
        ...
        for i, cat in enumerate(categories):
            if cat not in st.session_state[cache_key]:
                st.session_state[cache_key][cat] = get_sentence_transformer_embeddings(cat, model_name=model_name)
            cat_embed = st.session_state[cache_key][cat]
            cosine_sim[i] = cosine_similarity(sent_embed, cat_embed)
    ...
    ##########################################
    # Return list[(category_index, similarity_score)] sorted high->low
    sorted_items = sorted(cosine_sim.items(), key=lambda kv: kv[1], reverse=True)
    return sorted_items
...
if __name__ == "__main__":
   ...
   # )
    st.text_input(
        label="Categories", key="categories", value="Positive Negative"
    )
    ...
    text_search = st.text_input(
        ...
        value="This movie is absolutely terrible!",
    )
    ...
    if st.session_state.text_search:
        ...
        st.write("")
        st.write(
            "Demo developed by [Sheng Wei](https://linkedin.com/in/sheng-wei-0721)"
        )
